...
class Graph{
	...
	public:
		...
		void dijkstra(string a, string b, vector<string> path, string &minPath, int &weight){
			if(!exist(path,a)){
				path.push_back(a);
				...
				for(int i = 0;i < numVertex;i++)
					if(adjacent(n,i)){
						if(vertices[i].getName() == b){
							if(weight == 0){
								...
								for(vector<string>::iterator it = path.begin(); it != path.end();it++)
									minPath+=*it+"->";
								minPath+=b;
								weight = getWeight(path,b);
							}else{
								if(getWeight(path,b) < weight){
									minPath.clear();
									for(vector<string>::iterator it = path.begin(); it != path.end();it++)
										minPath+=*it+"->";
									minPath+=b;
						...
						}else dijkstra(vertices[i].getName(),b,path,minPath,weight);
					}
			}
		}
		
		void minimumPath(string a, string b){
			if(numOfVertex(a) > -1 && numOfVertex(b) > -1){
				vector<string> path;
				string minPath;
				int weight=0;
				cout<<"\nShowing the minimum path from \""<<a<<"\" to \""<<b<<"\"...\n";
				dijkstra(a,b,path,minPath,weight);
				weight != 0? cout<<minPath<<" (weight: "<<+weight<<")" : cerr<<"there's no path!";
			}
		}
};
